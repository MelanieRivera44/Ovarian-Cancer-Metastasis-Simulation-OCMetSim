;;Ovarian Cancer Metastasis Simulation-Spheroids
;;OCMetSIM-Spheroids
...
To Setup
  ...
  ;; Ovarian cancer cell spheroids will move according to adhesion and invasion parameters measured based on expression of Rac1 GTPase
  ;; Simulation represents ovarian cancer spheroids

  set CrowdingDeath 0  ;;This monitor counts the number of spheroids that die from crowding along the blood vessel
  ...
  ask patches with[pycor <= -12 and pycor >= -17]
  [
    set pcolor red
  ]
  ...
  create-turtles Initial_NumberofSpheroids ;;Changing this slider allows the user to change the number of cancer spheroid agents that are created in the world
  [
    set xcor random-xcor
    set ycor -9 + random 30 ;;Creating the cells at these coordinates, creates them specifically in the peritoneal compartment
    set shape "circle"
    set color yellow
  ]
...
To Go_AdhesionAndInvasionSpheroids ;;Button
  ;;The To Go_AdhesionAndInvasionSpheroids button runs the simulation commands to test ovarian cancer metastasis of spheroids to a distant tissue site with parameters for cellular adhesion and invasion based on Rac1 expression

  ;;Cells_MoveIn_Peritoneum- Ovarian cancer spheroids move in the peritoneal space
  ;;Cell_Adhesion_MesoClearanceData- Once the spheroids encounter the blood vessel, they are able to adhere based on experimental parameters set from mesothelial clearance experiments using cells with varying expression of Rac1
  ;;Cell_Invasion- When the spheroids adhere to the blood vessel, they are able to invade into ciruclation based on experimental parameters
  ;;Cells_ProbDeathInBlood- After the spheroids have invaded into the blood stream, some may die based on a probability of death in the blood stream
  ;;Cells_MoveIn_Blood- If the spheroids survive in the blood, they move toward the distant tissue site
  ;;Cells_AdhesionDistantTissue-Once the spheroids encounter the distant tissue, they are able to adhere based on experimental parameters set from in vitro experiments from cells with varying expression of Rac1
  ;;Cells_InvasionDistantTissue- When the spheroids have adhered to the distant tissue site, they are able to invade into the tissue based on experimental parameters
  ;;Cells_InDistantTissueSite- When the spheroid agents have invaded into the distant site, they change color and the simulation stops.  Ticks(time) and the number of spheroids can be quantified
  ;;Plots from each simulation run can also be saved as described below to track the number of spheroids in each compartment over each simulation run
  ...
  Cell_Adhesion_MesoClearanceData
  Cell_Invasion
  ...
  tick
  if not any? turtles with [color = yellow] ;;agents will change color to white during the last step and the simulation will stop if there are no more yellow agents left
  [
...
To Go_SpheroidCrowding ;;Button
  ;;The To Go_SpheroidCrowding button runs the simulation commands to test ovarian cancer metastasis to a distant tissue site while considering the possibility that cells can crowd along the blood vessel and cells can potentially die due to cell crowding and non-attachement
  ...
  ;;Cells_MoveIn_Peritoneum- Ovarian cancer spheroids move in the peritoneal space
  ;;Cell_Adhesion_withDeath_CrowdingEffectAdded_withCrowdingDeath- Once the spheroids encounter the blood vessel, they crowd the space around the blood vessel until the cells in front of eachother adhere and invade into the blood vessel
...
To Setup_duringRuns
  ...
  create-turtles Initial_NumberofSpheroids
  [
    set xcor random-xcor
    set ycor -9 + random 30
    set shape "circle"
    set color yellow
  ]
...
To Cells_MoveIn_Peritoneum
  ;;To Cells_MoveIn_Peritoneum dictates how the spheroids move in the peritoneal space
  ;;Ovarian cancer spheroid agents are created in this space
...
To Cell_Adhesion_MesoClearanceData
  ;;To Cell_Adhesion_MesoClearanceData is used to test adhesion with cell death and invasion into the blood stream and distant tissue site, while ignorning crowding along the blood vessel
  ;;Data from in vitro mesothelial clearance data was used to set parameters for adhesion and invasion parameters of ovarian cancer spheroids with varying Rac1 expression
  ;;Activated when Go_AdhesionAndInvasionSpheroids button is pushed
  ...
  ;;Once the ovarian cancer spheroid agents have moved around the peritoenal space and encounter the green patches, the ovarian cancer cells are able to adhere based on an adhesion probability parameter set based on Rac1 expression
  ;;The probability of adhesion, each timestep, that a spheroid adheres to the blood vessel
  ;;If the spheroids adhere, they move forward 1 into the green patches
  ;;If the spheroids do not adhere, they die due to non-attachement


  ask turtles
  [
    let colorofPatchAhead red
    if pcolor = black and pycor <= 19
    [
      ask patch-ahead 1
      [
        set colorofPatchAhead pcolor
...
To Cell_Adhesion_withDeath_CrowdingEffectAdded_withCrowdingDeath
  ...
  ;;Activated when Go_SpheroidCrowding button is pushed
    ...
    [
      ...
      [
        ...
        [
          ...
          [
            set Crowdingdeath Crowdingdeath + 1 ;;monitor to count the number of spheroids that die from cell crowding
            die ;;the spheroid will die due to cell crowding along the blood vessel
          ]
          [
            stop ;;the spheroid will wait to move until the patch ahead 1 is unoccupied
          ]
        ]
        [
        ifelse random 100 < probAdhesion
        [
          forward 1 ;;the spheroid will move forward 1 patch and officially adhere to the blood vessel based on the adhesion probability
          ]
          [
            die ;;the spheroid will die if it does not adhere to the blood vessel
          ]
        ]
      ]
    ]
...
To Cell_Invasion
  ;;Once the spheroids have adhered to the blood vessel, the rate/probability of invasion is activated
  ...
  [set heading 180
      ...
      set probInvasion random-poisson InvasionFromExp ;;Invasion variable
      if (probInvasion > 0)
      [
       forward 1
      ]
...
To Cells_ProbDeathInBlood
  ;;After the ovarian cancer spheroids have invaded into the blood stream, they have a probability of dying in the blood stream
  ;;Probability of cell death in the blood, each time step, when the spheroid first enters the blood stream (red patches)
...
To Cells_MoveIn_Blood
  ;;The ovarian cancer spheroids that have survived in the bloodstream will move forward 1 patch until they encounter patches for Adhesion to the Distant Tissue Site.
...
To Cells_AdhesionDistantTissue
  ;;After the ovarian cancer spheroids move through the blood and encounter the distant tissue site, they have a probability of adhering to the distant tissue
  ;;The probability of adhesion, each timestep, that a spheroid adheres to the distnat tissue site
  ;;If the spheroids adhere, they move forward 1 into the cyan patches
  ;;If the spheroids do not adhere, they die due to non-attachement

 ask turtles
  [
    let colorofPatchAhead red
    if pcolor = red and pycor = -17
    [
      ask patch-ahead 1
      [
        set colorofPatchAhead pcolor
...
To Cells_InvasionDistantTissue
  ;;Once the ovarian cancer spheroids have adhered to the distant tissue site, the rate/probability of invasion is activated
  ;;Invasion parameters are set based on experimental data from in vitro experiments from cells with varying expression of Rac1
  ;;The probability, each time step, that a spheroid invades into the distant tissue site once adhered
  ...
  [
    ...
    [
      set heading 180
      set probInvasion random-poisson InvasionFromExp
      if (probInvasion > 0)
      [
        forward 1
      ]
...
To Cells_InDistantTissueSite
  ...
  ;;Once the ovarian cancer spheroid agents invade into the distant tissue site, they turn white and step forward into the distant tissue compartment and the simulation stops
  ;;The number of spheroids in the distant tissue site is counted by a monitor
  ;;The number of ticks at the top represents overall time it took for the spheroids to reach the distant tissue site
...
84
NIL
Go_AdhesionAndInvasionSpheroids
T
1
T
OBSERVER
...
473
105
# of cells invaded into the blood vessel
...
count turtles with [pcolor = magenta]
17
1
11
...
This program simulates ovarian cancer spheroids. 

## HOW IT WORKS

This program simulates ovarian cancer spheroids.  The world size and set up is 1/3 the size of the program OCMetSIM-Single Cells. This allows the user to simulate metastasis of spheroids vs single cells, while keeping the scales similar.  In this program, a turtle or ovarian cancer spheroid occupies one patch which represents a spheroid cluster of 3x3 cells- a spheroid with 9 cells. The number of spheroids used in OCMetSIM-Spheroids should be 9x less than the OCMetSIM-Single Cell initial cell count to keep the scales similar.
...
1. Set Initial_NumberOfCells slider to the desired spheroid number
...
6. In the code tab, under the 'To Go_AdhesionAndInvasionSpheroids' procedure, setup a file path with a folder to save the plot data 
7. Click the Setup Button
8. Click the Go_AdhesionAndInvasionSpheroids
...
1. Set Initial_NumberOfCells slider to the desired number spheroid number
2. Set appropriate probability of adhesion parameter
3. Set appropriate probability/rate of invasion parameter
4. Set appropriate probability of death in the blood
...
7. In the code tab, under the 'To Go_SpheroidCrowding' procedure, setup a file path with a folder to save the plot data
8. Click the Setup Button
9. Click the Go_SpheroidCrowding
